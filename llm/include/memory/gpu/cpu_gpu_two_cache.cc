...
struct MSHREntry {
    ...
    MSHREntry() : address(0), requestType(READ), requestTime(SC_ZERO_TIME), pendingTransaction(nullptr), isPending(false) {}
};
...
public:
    ...
    //     std::cout << "L1-" << cacheId << ": Queueing writeback for 0x" <<
    ...
    void processRequests() {
        while (true) {
            ...
            if (request.requestType == READ || request.requestType == WRITE) {
                ...
                tlm_phase phase = BEGIN_REQ;
                sc_time delay = SC_ZERO_TIME;
                l2_socket->nb_transport_fw(*request.transaction, phase, delay);
    ...
    void generateTraffic() {
        ...
        for (int i = 0; i < 10; i++) {
            ...
            if (isWrite) {
                ...
                cout << name() << ": Writing to address " << hex << addr << endl;
            } else {
                // 读请求
                ...
                cout << name() << ": Reading from address " << hex << addr << endl;
            }
