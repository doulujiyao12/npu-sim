...
template <class T, unsigned BANK_NUM> class DynamicBandwidthRamRow : public sc_channel, public multiport_read_if<T, BANK_NUM>, public ram_time_if<T> {
public:
    SC_HAS_PROCESS(DynamicBandwidthRamRow);
    DynamicBandwidthRamRow(sc_module_name name_, uint64_t start_address, uint64_t depth_per_bank, uint64_t read_port_per_bank, uint64_t write_port_per_bank, int read_port_num, int write_port_num,
                           int hb_read_port_num, Event_engine *_e_engine)
        ...
        for (auto i = 0; i < BANK_NUM; i++) {
            ...
            ram_banks_.push_back(new ArbiterRamBank<T>(ram_name, 0, depth_per_bank_, read_port_per_bank_, write_port_per_bank_, e_engine_));
        ...
        high_bw_read_semaphore_ = new sc_semaphore(1);  // only support 1 request at a time
        high_bw_write_semaphore_ = new sc_semaphore(1); // only support 1 request at a time
...
public:
    ...
    uint64_t bound_read_port_num_, bound_write_port_num_, bound_high_bw_read_port_num_;
...
public:
    ...
    transfer_status read(uint64_t address, T &data, sc_time &elapsed_time, bool shadow = false);
    transfer_status multiport_read(uint64_t address_in_high_bitwidth, vector<T> &data, sc_time &elapsed_time);
    transfer_status multiport_write(uint64_t address_in_high_bitwidth, vector<T> &data, sc_time &elapsed_time);
    transfer_status write(uint64_t address, T &data, sc_time &elapsed_time, bool force_write = 1, bool shadow = false);
...
template <class T, unsigned BANK_NUM> inline DynamicBandwidthRamRow<T, BANK_NUM>::~DynamicBandwidthRamRow() {
...
template <class T, unsigned BANK_NUM> inline uint64_t DynamicBandwidthRamRow<T, BANK_NUM>::get_bank_index(uint64_t address) {
    uint64_t column_idx = (address - low_bw_start_address_) % BANK_NUM;
    return column_idx;
}

template <class T, unsigned BANK_NUM> inline uint64_t DynamicBandwidthRamRow<T, BANK_NUM>::get_local_address(uint64_t address) {
    uint64_t row_relative_address = (address - low_bw_start_address_) % (depth_per_bank_ * BANK_NUM);
    return row_relative_address / (BANK_NUM);
}

template <class T, unsigned BANK_NUM> inline void DynamicBandwidthRamRow<T, BANK_NUM>::register_port(sc_port_base &port_, const char *if_typename_) {
    ...
    if (nm == typeid(mem_read_if<T>).name()) {
        ...
        assert(bound_read_port_num_ < read_port_num_ || read_port_num_ < 0);
        assert(bound_high_bw_read_port_num_ < hb_read_port_num_ || hb_read_port_num_ < 0); // only support one high-bw port for now
        bound_read_port_num_ += BANK_NUM;
        bound_high_bw_read_port_num_++;
    }
}
// address 是 index
template <class T, unsigned BANK_NUM> inline transfer_status DynamicBandwidthRamRow<T, BANK_NUM>::read(uint64_t address, T &data, sc_time &elapsed_time, bool shadow) {
    if (address < low_bw_start_address_ || address > low_bw_end_address_) {
        return TRANSFER_ERROR;
    }
    sc_time start_time = sc_time_stamp();
    uint64_t bank_index = get_bank_index(address);
    transfer_status temp_status = ram_banks_[bank_index]->read(get_local_address(address), data, shadow);
    elapsed_time = sc_time_stamp() - start_time;
    return temp_status;
}


template <class T, unsigned BANK_NUM> inline transfer_status DynamicBandwidthRamRow<T, BANK_NUM>::multiport_write(uint64_t address_in_high_bitwidth, vector<T> &data, sc_time &elapsed_time) {
    // 检查数据大小与银行数量是否匹配
...
#if VERBOSE_TRACE == 1
    e_engine_->add_event(this->name(), __func__, "B", Trace_event_util(sc_get_current_process_b()->name()));
#endif
    wait(event_all_done);
#if VERBOSE_TRACE == 1
    e_engine_->add_event(this->name(), __func__, "E", Trace_event_util(sc_get_current_process_b()->name()));
#endif
    // 释放信号量
...
// 一次性读出所有bank的数据，可以跨行
template <class T, unsigned BANK_NUM> inline transfer_status DynamicBandwidthRamRow<T, BANK_NUM>::multiport_read(uint64_t address_in_high_bitwidth, vector<T> &data, sc_time &elapsed_time) {
    assert(data.size() == BANK_NUM);
    sc_time start_time = sc_time_stamp();
    if (high_bw_read_semaphore_->trywait() == -1) {
        high_bw_read_semaphore_->wait(); // if the mutex is locked, this function will wait
    }
...
template <class T, unsigned BANK_NUM> inline transfer_status DynamicBandwidthRamRow<T, BANK_NUM>::write(uint64_t address, T &data, sc_time &elapsed_time, bool force_write, bool shadow) {
    if (address < low_bw_start_address_ || address > low_bw_end_address_) {
        return TRANSFER_ERROR;
    }
    sc_time start_time = sc_time_stamp();
    uint64_t bank_index = get_bank_index(address);
    transfer_status temp_status = ram_banks_[bank_index]->write(get_local_address(address), data, force_write, shadow);
    elapsed_time = sc_time_stamp() - start_time;
    return temp_status;
}

template <class T, unsigned BANK_NUM> inline transfer_status DynamicBandwidthRamRow<T, BANK_NUM>::clear(uint64_t address, T &data) {
    if (address < low_bw_start_address_ || address > low_bw_end_address_) {
        return TRANSFER_ERROR;
    }
    uint64_t bank_index = get_bank_index(address);
    transfer_status temp_status = ram_banks_[bank_index]->clear(get_local_address(address), data);
    return temp_status;
}

template <class T, unsigned BANK_NUM> inline bool DynamicBandwidthRamRow<T, BANK_NUM>::reset() {
    assert(bound_read_port_num_ < read_port_num_ && bound_write_port_num_ < write_port_num_ && bound_high_bw_read_port_num_ < hb_read_port_num_);
    for (auto i = 0; i < BANK_NUM; i++) {
        ram_banks_[i]->reset();
    }
    return true;
...
template <class T, unsigned BANK_NUM> inline uint64_t DynamicBandwidthRamRow<T, BANK_NUM>::start_address() const { return low_bw_start_address_; }

template <class T, unsigned BANK_NUM> inline uint64_t DynamicBandwidthRamRow<T, BANK_NUM>::end_address() const { return low_bw_end_address_; }
