...
void Matmul_f_prefill::print_self(string prefix) {
    cout << prefix << "<matmul_forward_prefill>\n";
...
void Matmul_f_prefill::initialize() {
    out_size = B * T * OC;
    p_inp_size = B * T * C;
...
void Matmul_f_prefill::parse_json(json j) {
    B = find_var(j["B"]);
    T = find_var(j["T"]);
    C = find_var(j["C"]);
    OC = find_var(j["OC"]);
...
int Matmul_f_prefill::sram_utilization(DATATYPE datatype) {
    int total_sram = 0;
    int data_byte = 0;
    if (datatype == INT8) {
        data_byte = 1;
...
void Matmul_f_prefill::deserialize(sc_bv<128> buffer) {
...
sc_bv<128> Matmul_f_prefill::serialize() {
    sc_bv<128> d;
    d.range(7, 0) = sc_bv<8>(0x14);
...
void Matmul_f_prefill::matmul_forward_naive(float *out, const float *inp, const float *weight, const float *bias, int B, int T, int C, int OC) {
...
int Matmul_f_prefill::task_core(TaskCoreContext &context) {
...
// 使用 context 访问原变量
    ...
    printf("matmul_forward_prefill: dram time 1: %ld\n", dram_time);
    ...
    for (int batch = 0; batch < B; batch++) {
        ...
        sram_write_append_generic(context, data_byte * data_size_out / 3, dram_time);
    ...
    SramPosKey out_key = SramPosKey(*sram_addr, data_byte * data_size_out / 3);
...
int Matmul_f_prefill::task() {
    ...
    for (int oc = 0; oc < OC; oc++) {
        for (int c = 0; c < C; c++) {
            ...
            dram_time += check_dcache(0, 0, weight_dcacheline << (dcache_words_in_line_log2 + 2), dram_time, time_fetched, time_prefetched, prefetch_tag, false);
    ...
    for (int oc = 0; oc < OC; oc++) {
        ...
        dram_time += check_dcache(0, 0, bias_dcacheline << (dcache_words_in_line_log2 + 2), dram_time, time_fetched, time_prefetched, prefetch_tag, false);
