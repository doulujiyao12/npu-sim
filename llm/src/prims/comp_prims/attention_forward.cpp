...
void Attention_f::parseJson(json j) {
    /*
    ...
    */
    SetParamFromJson(j, "B", &B);
    SetParamFromJson(j, "T", &T);
    SetParamFromJson(j, "C", &C);
    SetParamFromJson(j, "NH", &NH);
    SetParamFromJson(j, "R", &R);
...
int Attention_f::sram_utilization(DATATYPE datatype, int cid) {
    ...
    int p_inp_sram = CeilingDivision(B * T * 3 * C * data_byte * 8,
                                     GetCoreHWConfig(cid).sram_bitwidth);
    int a_sram = CeilingDivision(B * NH * T * T * data_byte * 8,
                                 GetCoreHWConfig(cid).sram_bitwidth);
    int out_sram = CeilingDivision(out_size * data_byte * 8,
                                   GetCoreHWConfig(cid).sram_bitwidth);

    total_sram = (p_inp_sram + a_sram + out_sram) *
                 GetCoreHWConfig(cid).sram_bitwidth / 8;
...
int Attention_f::task_core(TaskCoreContext &context) {
    ...
    writeOutputData(context,
                    (uint64_t)B * NH * T * (T - 1) / 2 * (4 * C / NH + 5), 0,
