...
bool is_comp_prim(prim_base *p) {
    if (typeid(*p) == typeid(Attention_f) || typeid(*p) == typeid(Conv_f) || typeid(*p) == typeid(Max_pool) || typeid(*p) == typeid(Dummy_p) || typeid(*p) == typeid(Gelu_f) ||
        typeid(*p) == typeid(Layernorm_f) || typeid(*p) == typeid(Matmul_f) || typeid(*p) == typeid(Relu_f) || typeid(*p) == typeid(Residual_f) || typeid(*p) == typeid(Batchnorm_f) ||
        typeid(*p) == typeid(Attention_f_decode) || typeid(*p) == typeid(Matmul_f_decode) || typeid(*p) == typeid(Matmul_f_prefill) || typeid(*p) == typeid(Merge_matmul) ||
        typeid(*p) == typeid(Split_matmul) || typeid(*p) == typeid(Merge_conv) || typeid(*p) == typeid(Split_conv))
        return true;
