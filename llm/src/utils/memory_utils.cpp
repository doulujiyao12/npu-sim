...
void sram_first_write_generic(TaskCoreContext &context, int data_size_in_byte, int global_addr, u_int64_t &dram_time, float *dram_start) {
    int dma_read_count = data_size_in_byte * 8 / (int)(SRAM_BITWIDTH * SRAM_BANKS);
    int byte_residue = data_size_in_byte * 8 - dma_read_count * (SRAM_BITWIDTH * SRAM_BANKS);
    ...
    for (int i = 0; i < dma_read_count; i++) {
        ...
        hmau->mem_read_port->multiport_write(sram_addr_temp, data_tmp, elapsed_time);
    ...
    dram_time += (end_first_write_time - start_first_write_time).to_seconds() * 1e9;
...
void sram_spill_back_generic(TaskCoreContext &context, int data_size_in_byte, int global_addr, u_int64_t &dram_time) {
    int dma_read_count = data_size_in_byte * 8 / (int)(SRAM_BITWIDTH * SRAM_BANKS);
    int byte_residue = data_size_in_byte * 8 - dma_read_count * (SRAM_BITWIDTH * SRAM_BANKS);
    int single_read_count = ceiling_division(byte_residue, SRAM_BITWIDTH);
    int inp_global_addr = global_addr;
    ...
    for (int i = 0; i < dma_read_count; i++) {
        sc_time elapsed_time;
        hmau->mem_read_port->multiport_read(sram_addr_temp, data_tmp, elapsed_time);
        u_int64_t sram_timer = elapsed_time.to_seconds() * 1e9;
        sram_time += sram_timer;
        sram_addr_temp = sram_addr_temp + SRAM_BANKS;
    }
...
void sram_read_generic(TaskCoreContext &context, int data_size_in_byte, int sram_addr_offset, u_int64_t &dram_time) {
    int dma_read_count = data_size_in_byte * 8 / (int)(SRAM_BITWIDTH * SRAM_BANKS);
    int byte_residue = data_size_in_byte * 8 - dma_read_count * (SRAM_BITWIDTH * SRAM_BANKS);
    int single_read_count = ceiling_division(byte_residue, SRAM_BITWIDTH);
    ...
    for (int i = 0; i < dma_read_count; i++) {
        sc_time elapsed_time;
        hmau->mem_read_port->multiport_read(sram_addr_offset, data_tmp, elapsed_time);
        u_int64_t sram_timer = elapsed_time.to_seconds() * 1e9;
        dram_time += sram_timer;
...
void sram_write_append_generic(TaskCoreContext &context, int data_size_in_byte, u_int64_t &dram_time) {
    int dma_read_count = data_size_in_byte * 8 / (int)(SRAM_BITWIDTH * SRAM_BANKS);
    int byte_residue = data_size_in_byte * 8 - dma_read_count * (SRAM_BITWIDTH * SRAM_BANKS);
    int single_read_count = ceiling_division(byte_residue, SRAM_BITWIDTH);
...
void sram_write_back_generic(TaskCoreContext &context, int data_size_in_byte, int sram_addr, u_int64_t &dram_time) {
    int dma_read_count = data_size_in_byte * 8 / (int)(SRAM_BITWIDTH * SRAM_BANKS);
    int byte_residue = data_size_in_byte * 8 - dma_read_count * (SRAM_BITWIDTH * SRAM_BANKS);
    int single_read_count = ceiling_division(byte_residue, SRAM_BITWIDTH);
...
void gpu_read_generic(TaskCoreContext &context, uint64_t global_addr, int data_size_in_byte, int &mem_time) {
    ...
    int inp_global_addr = (global_addr / 64) * 64; // 向下取整到dram 取址的整数倍，这里是32
