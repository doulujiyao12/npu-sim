...
void config_helper_pd::fill_queue_start(queue<Msg> *q) {
    for (auto &status : coreStatus) {
        ...
        for (auto request : status.reqs) {
            ...
            switch (record.phase) {
            case PREFILL:
                size += record.prefill_counter * record.seq_len /
                        record.prefill_iters * record.heads * 64;
                break;
            case DECODE:
                size += 1 * record.heads * 64;
                break;
            }
        }
...
void config_helper_pd::schedule() {
    ...
    for (auto status : coreStatus) {
        if (status.available) {
            // 分配新的request batch
            vector<RequestRecord> new_reqs;
            for (auto req : status.reqs) {
                auto &record = requestRecords[req];
                if (record.phase != PD_DONE) {
                    record.lock = true;
                    new_reqs.push_back(record);
                }
            }
...
void config_helper_pd::generate_prims(int i) {
    // 根据template json组织新的config，在每一个计算原语之间插入set_pd_work原语，用于告知混合batch内容
}